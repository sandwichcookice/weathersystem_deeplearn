    ...
    for region in batch_data:
        ...
        for ep in tqdm(range(num_epochs), desc=f"訓練 {region_id}"):
                ...
                if len(buffer) >= learning_starts:
                    try:
                        ...
                        with torch.no_grad():
                            ...
                            # DQN 直接從目標網路中取最大 Q 值：
                            next_q, _ = next_q_vals_target.max(dim=2)
                            next_q = next_q.mean(dim=1)
            ...
            if global_epoch % 1000 == 0:
                ...
                checkpoint_dir = os.path.join(base_dir, "..", "model", "dqn")
                os.makedirs(checkpoint_dir, exist_ok=True)
                torch.save(model.state_dict(), os.path.join(checkpoint_dir, f"dqn_checkpoint_ep{global_epoch}.pth"))
